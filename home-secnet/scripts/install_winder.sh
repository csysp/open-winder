...
# Purpose: Bootstrap open-winder v0.x on a Proxmox host
# Inputs: none (interactive setup wizard will prompt)
# Outputs: installs repo under /root/open-winder and runs setup
...
cd /root
if [[ -d /root/open-winder ]]; then
  echo "Existing /root/open-winder present. Move or remove it before installing." >&2
  exit 1
fi

curl -fsSL -o open-winder-v0.1.1.tar.gz https://github.com/csysp/winder/archive/refs/tags/v0.1.1.tar.gz
tar -xzf open-winder-v0.1.1.tar.gz
mv winder-0.1.1 open-winder
cd open-winder
