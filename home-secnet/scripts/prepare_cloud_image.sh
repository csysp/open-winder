...
echo "[03] Fetching Ubuntu 24.04 cloud image and creating template..."
...
echo "[03] Using storages: ISO=$ISO_STORAGE, DISK=$DISK_STORAGE"

TEMPLATE_VM_ID=9000
if qm status $TEMPLATE_VM_ID >/dev/null 2>&1; then
  echo "[03] Template VM $TEMPLATE_VM_ID already exists. Skipping."
  exit 0
fi
...
if ! command -v curl >/dev/null 2>&1; then
  echo "[03] Installing curl..."
  apt-get update -y && apt-get install -y curl
fi
if [[ ! -f "$IMG_FILE" ]]; then
  echo "[03] Downloading Ubuntu 24.04 cloud image... ($IMG_URL)"
  curl -L "$IMG_URL" -o "$IMG_FILE"
fi

echo "[03] Creating cloud-init template VM $TEMPLATE_VM_ID..."
...
echo "[03] Template created (VMID=$TEMPLATE_VM_ID)."
