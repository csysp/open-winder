...
echo "[06] Rendering router configs from .env and generating keys..."
...
if [[ ! -f "$WG_DIR/privatekey" ]]; then
  echo "[06] Generating WireGuard keypair..."
  umask 077
...
# Autogenerated by 06_generate_router_configs.sh
...
render() {
  ...
  if command -v envsubst >/dev/null 2>&1; then
    ...
    perl -M5.010 -pe 's/\$\{([A-Z0-9_]+)\}/$ENV{$1} //ge' "$src" > "$dst"
  fi
  set -e
}
...
EOF
  echo "[06] QUIC wrapper enabled. Use hysteria2 client to wrap WG UDP over QUIC."
fi
...
if [[ "${DOUBLE_HOP_ENABLE:-false}" == "true" ]]; then
  ...
  echo "[06] Double-hop enabled. Remember to configure the exit node peer to accept ${WG2_ADDRESS}."
fi
...
cat > "$NP_OUT" <<EOF
# Generated by 06_generate_router_configs.sh
network:
  version: 2
  renderer: networkd
  ethernets:
...
echo "[06] Render complete. Artifacts under render/ and clients/."
